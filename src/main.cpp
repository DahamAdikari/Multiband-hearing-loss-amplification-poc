...
static bool read_wav_mono16(const std::string& path, Wav& out) {
    ...
    if (numChannels != 1) return false;     // mono
    if (bitsPerSample != 16) return false;  // 16-bit
...
static bool write_wav_mono16(const std::string& path, const std::vector<float>& x, int sampleRate) {
...
struct Compressor {
    ...
    void init(float sampleRate) { fs = sampleRate; env = 0.0f; }
    ...
    inline float process(float x) {
        ...
        if (ax > env) env = attackCoef * env + (1 - attackCoef) * ax;
        else          env = releaseCoef * env + (1 - releaseCoef) * ax;
...
// --------- Export-only band writer (one input WAV -> results/<stem>_multibands/*.wav) ----------
static bool export_bands_only(
    const Wav& wav,
    const std::string& inPath,
    const std::string& mode,          // static|compress|broadband (broadband treated like "on" stage on fullband; for bands use static/compress)
    const float makeupDb[6],
    Compressor comp[6],
    ...
    for (int i = 0; i < 6; i++) band[i].resize(wav.x.size());
    ...
    for (int i = 0; i < 6; i++) {
        fs::path outFile = outDir / (stem + "_band" + std::to_string(i + 1) + "_" + stage + ".wav");
    ...
    std::cout << "Stage: " << stage << " (in=raw split, on=after gain/compress)\n";
    return true;
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--out_off" && i+1 < argc) outOffPath = argv[++i];
        else if (a == "--out_on" && i+1 < argc) outOnPath = argv[++i];
        else if (a == "--loss" && i+1 < argc) {
            hasLoss = parse_csv6(argv[++i], loss6);
            if (!hasLoss) { std::cerr << "Bad --loss format.\n"; return 1; }
        }
        ...
        else if (a == "--ratio" && i+1 < argc) ratio = std::stof(argv[++i]);
        else if (a == "--attack_ms" && i+1 < argc) attackMs = std::stof(argv[++i]);
        else if (a == "--release_ms" && i+1 < argc) releaseMs = std::stof(argv[++i]);
        else if (a == "--thr" && i+1 < argc) {
            float t6[6];
            if (!parse_csv6(argv[++i], t6)) { std::cerr << "Bad --thr format.\n"; return 1; }
            for (int k = 0; k < 6; k++) thrDb[k] = t6[k];
        }
        else if (a == "--master" && i+1 < argc) masterDb = std::stof(argv[++i]);
        else if (a == "--wet" && i+1 < argc) wet = std::stof(argv[++i]);
        else if (a == "--dry" && i+1 < argc) dry = std::stof(argv[++i]);

        // export
        else if (a == "--export_bands") exportBands = true;
        else if (a == "--export_root" && i+1 < argc) exportRoot = argv[++i];
            ...
            std::cout <<
                "Usage (normal):\n"
                "  hearing_phase4_poc --in <input.wav> [--loss \"10,15,25,35,45,50\"]\n"
                "                   [--mode compress|static|broadband]\n"
                "                   [--out_off off.wav] [--out_on on.wav]\n"
                "                   [--ratio 4] [--attack_ms 20] [--release_ms 250]\n"
                "                   [--thr \"-18,-22,-26,-30,-34,-36\"] [--master 0]\n"
                "                   [--dry 1] [--wet 1]\n\n"
                "Usage (export bands only):\n"
                "  hearing_phase4_poc --in <input.wav> --export_bands [--stage in|on]\n"
                "                   [--export_root results] [--mode compress|static] [--loss \"...\"]\n\n"
                "Bands:\n"
    ...
    if (hasLoss) loss_to_makeup_db(loss6, makeupDb);

    // broadband gain from average loss (half-gain, capped)
    float broadbandGainDb = 0.0f;
    if (hasLoss) {
        float sumLoss = 0.0f;
        for (int i = 0; i < 6; i++) sumLoss += loss6[i];
    ...
    if (exportBands) {
        if (!export_bands_only(wav, inPath, mode, makeupDb, comp, exportRoot, exportStage)) return 1;
        return 0;
    }
    ...
    std::cout << "Bands:      B1:0-500, B2:500-1k, B3:1k-2k, B4:2k-4k, B5:4k-8k, B6:8k+\n";
    if (mode == "broadband") {
